...
const EVENT_SLUG_LOOKUP_NOTE = "Run `npm run seed` first, then replace EVENT_ID below with the seeded event's id.";
...
async function registerAndLogin(index: number): Promise<string> {
  ...
  await fetch(`${BASE_URL}/auth/register`, {
    ...
    body: JSON.stringify({ email, password: "TestPass123!", name: `Load Test ${index}`, role: "ATTENDEE" }),
  });
...
async function attemptBooking(token: string, index: number) {
  ...
  return { index, status: res.status, success: res.status === 201, code: body.code };
}

async function main() {
  if (!EVENT_ID) {
    console.error(EVENT_SLUG_LOOKUP_NOTE);
    console.error("Set EVENT_ID env var to the seeded event's id (check `npm run prisma:studio` or the seed log) and re-run.");
    process.exit(1);
  }

  console.log(`Firing ${CONCURRENT_REQUESTS} concurrent booking requests against tier "${TICKET_TIER_ID}"...`);

  const tokens = await Promise.all(
    Array.from({ length: CONCURRENT_REQUESTS }, (_, i) => registerAndLogin(i))
  );

  const results = await Promise.all(tokens.map((token, i) => attemptBooking(token, i)));

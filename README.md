...
This project runs as native ES Modules (`"type": "module"` in package.json) via [`tsx`](https://tsx.is/), both in dev and for `npm start`. There's no `tsc`-emitted `dist/` folder in the normal flow — `tsx` transpiles and runs TypeScript directly. `npm run typecheck` (`tsc --noEmit`) is still there for pure type-checking in CI/editors.

**All imports use real relative paths (`../config/db`, not `@/config/db`).** An earlier version of this project used a `@/*` tsconfig path alias, which worked in initial testing but then failed on a real machine (Node 24) with `Cannot find package '@/generated'` — Node's ESM resolver treated the alias as if it were a scoped npm package name (`@scope/name` syntax) rather than a TypeScript-only convenience, and `tsx`'s own alias-resolution shim didn't reliably intercept it in that environment either. Runtime tsconfig-paths resolution turns out to be inconsistent across Node versions/OSes industry-wide, not specific to this setup — so it was removed entirely in favor of relative imports, which need no special resolution logic beyond what Node has always supported natively. `tsconfig.json` no longer declares a `paths` mapping.
...
- The client generates to a **custom path** (`src/generated/prisma`, per `prisma/schema.prisma`'s `generator` block) instead of `node_modules/@prisma/client` — every import in this codebase uses a relative path to that folder, not the bare `@prisma/client` specifier.
...
This project was built in a sandbox without access to `binaries.prisma.sh`, so `prisma generate` can't complete here — meaning `src/generated/prisma` doesn't exist in this environment, and `npm run typecheck` shows a wall of "Cannot find module '../generated/prisma'" errors that are **not real bugs**, just the missing generated output. Every one of those disappears once you run `npx prisma generate` somewhere with normal internet access.
...
Two genuine bugs _were_ found and fixed this way before delivery:

...
async function register(input: RegisterInput): Promise<{ user: AuthUserResponse } & AuthTokens> {
  const existing = await prisma.user.findUnique({ where: { email: input.email } });
  if (existing) {
    throw ApiError.badRequest("Validation failed", [{ field: "email", message: "Email already exists" }]);
  }
...
async function login(input: LoginInput): Promise<{ user: AuthUserResponse } & AuthTokens & { expiresIn: number }> {
...
async function refreshToken(token: string): Promise<AuthTokens & { expiresIn: number }> {
...
async function forgotPassword(email: string): Promise<void> {
  ...
  const resetToken = jwt.sign({ id: user.id, purpose: "password_reset" }, env.jwt.refreshSecret, {
    expiresIn: "15m",
  });
...
async function resetPassword(token: string, newPassword: string): Promise<void> {
  ...
  try {
    decoded = jwt.verify(token, env.jwt.refreshSecret) as { id: string; purpose: string };
  } catch {
    throw ApiError.badRequest("Reset token is invalid or expired");
  }
  if (decoded.purpose !== "password_reset") throw ApiError.badRequest("Invalid reset token");

  const hashed = await hashPassword(newPassword);
  await prisma.user.update({ where: { id: decoded.id }, data: { password: hashed } });
}

export const authService = { register, login, refreshToken, forgotPassword, resetPassword };

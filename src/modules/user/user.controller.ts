...
const uploadProfileImage = catchAsync(async (req: Request, res: Response) => {
  if (!req.file) throw ApiError.badRequest("No image file provided", [{ field: "image", message: "Image is required" }]);
  const profileImage = await userService.uploadProfileImage(req.user!.id, req.file.buffer);
...
export const userController = { getMe, updateMe, uploadProfileImage, changePassword, getPublicProfile };
